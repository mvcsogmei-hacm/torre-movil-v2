...
- **Indicador principal de Inicio** (desde 18-ago-2026): título "Sector", subtítulo "Vivienda, Construcción y Saneamiento"; el % grande sale de la nueva hoja `INDICADOR PRINCIPAL` (fila SECTOR → `INDICADORES.principal.pct`, hoy 70,8% → **redondeado a entero** 71%), sin línea secundaria y sin botón de ojo. El botón dice **"Ver ranking"** (sigue abriendo `screen-pliego`). De los 3 botones inferiores, el primero se llama **"Pliego MVCS"**. La fila PLIEGO de la hoja `PLIEGO` (`INDICADORES.pliego[0]`) queda solo como resumen del detalle de Pliego; el desglose Actividades/Proyectos con decimales vive solo en el detalle.
...
- **Tarjetas "Pliegos"** (antes "Entidades adscritas", desde 18-ago-2026): hoja **`PLIEGOS`** (`INDICADORES.pliegos`, columnas ENTIDAD/PIM/DEVENGADO/EJECUCIÓN): 5 tarjetas — MVCS (ancho completo, `.ent.wide`), SBN, SENCICO, OTASS, COFOPRI — con % de ejecución redondeado, barra y "S/ dev M de S/ PIM M". La celda de ejecución es fórmula: el generador abre el Excel con `data_only=True` y, si viene vacía, calcula dev/PIM. La hoja `ADSCRITAS` sigue exportándose (`INDICADORES.adscritas`) pero ya no se muestra.
- La hoja del detalle de Pliego se llama ahora **`MVCS`** (antes `PLIEGO`; el generador acepta ambas) y sigue exportándose como `INDICADORES.pliego`.
