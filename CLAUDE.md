...
- **Indicador principal de Inicio** (desde 18-ago-2026): título "Sector", subtítulo "Vivienda, Construcción y Saneamiento"; el % grande sale de la nueva hoja `INDICADOR PRINCIPAL` (fila SECTOR → `INDICADORES.principal.pct`, hoy 70,8% → **redondeado a entero** 71%), sin línea secundaria y sin botón de ojo. El botón **"Ver ranking"** (y toda la parte roja de la tarjeta) abre la **pantalla Ranking** (`screen-ranking`, nueva 18-ago-2026): `balance-card` con "Sector / Vivienda, Construcción y Saneamiento", % con decimal, barra, y debajo en líneas separadas PIM y Devengado (filas PIM y DEVENGADO de la misma hoja `INDICADOR PRINCIPAL` → `INDICADORES.principal.{pct,pim,devengado}`); Debajo, sección **"Ranking total Gobierno Nacional"** con segmentador de 2 botones (**Por ejecución presupuestal** / **Por PIM**) y **podio** (diseño 2 de `Propuestas/galeria-ranking.html`, elegido 18-ago-2026) con el **Top 5** de los 30 sectores de la hoja **`RANKING SECTOR`** (`INDICADORES.rankingSector[{cod,nombre,pim,pct}]`; se muestra "NN: NOMBRE" con el código del pliego): Top 3 como podio 2-1-3 (`.podio .p`, medallas oro/plata/bronce, el 1.º más alto) y 4.º/5.º en `detail-card.rank-lista` con posición en círculo; valor = % con semáforo o PIM (en el podio abreviado: ≥1,000 M → "S/ 47,6 MM", si no "S/ 736 M", una sola línea; en la lista "S/ N M" completo); tarjetas del podio de altura fija (nombre limitado a 3 líneas); ordenado desc según el botón; VIVIENDA va en rojo (`.p.mio` / `.dr.mio`) y, si queda fuera del Top 5, se añade al final de la lista tras "···" con su puesto real. Debajo, con la **misma lógica** (segmentador propio + Top 5 + Vivienda): **"Ranking por inversiones"** (hoja `RANKING INVERSIONES` → `INDICADORES.rankingInversiones`) y **"Ranking por actividades"** (hoja `RANKING ACTIVIDADES` → `INDICADORES.rankingActividades`). Todo lo monta `montarRanking(lista, prefijo)` con ids `<prefijo>-seg` / `<prefijo>-tabla` (`rank`, `rankInv`, `rankAct`). De los 3 botones inferiores, el primero se llama **"Pliego MVCS"**. La fila PLIEGO de la hoja `MVCS` (`INDICADORES.mvcs[0]`) queda solo como resumen del detalle de Pliego MVCS; el desglose Actividades/Proyectos con decimales vive solo en el detalle.
