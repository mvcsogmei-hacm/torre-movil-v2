...
- **Indicador principal de Inicio** (desde 18-ago-2026): título "Sector", subtítulo "Vivienda, Construcción y Saneamiento"; el % grande sale de la nueva hoja `INDICADOR PRINCIPAL` (fila SECTOR → `INDICADORES.principal.pct`, hoy 70,8% → **redondeado a entero** 71%), sin línea secundaria y sin botón de ojo. El botón **"Ver ranking"** (y toda la parte roja de la tarjeta) abre la **pantalla Ranking** (`screen-ranking`, nueva 18-ago-2026): `balance-card` con "Sector / Vivienda, Construcción y Saneamiento", % con decimal, barra, y debajo en líneas separadas PIM y Devengado (filas PIM y DEVENGADO de la misma hoja `INDICADOR PRINCIPAL` → `INDICADORES.principal.{pct,pim,devengado}`); Debajo, sección **"Ranking total Gobierno Nacional"** con segmentador de 2 botones (**Por ejecución presupuestal** / **Por PIM**) y tabla `detail-card.rank-card` **Top 5** de los 30 sectores de la hoja **`RANKING SECTOR`** (`INDICADORES.rankingSector[{cod,nombre,pim,pct}]`; se muestra "NN: NOMBRE" con el código del pliego): posición en círculo, nombre, valor (% con semáforo o "S/ N M" con barra relativa al mayor PIM), ordenada desc según el botón; la fila VIVIENDA va resaltada (`.drb.mio`) y, si queda fuera del Top 5, se añade al final tras un separador "···" con su puesto real. Debajo, con la **misma lógica** (segmentador propio + Top 5 + Vivienda): **"Ranking por inversiones"** (hoja `RANKING INVERSIONES` → `INDICADORES.rankingInversiones`) y **"Ranking por actividades"** (hoja `RANKING ACTIVIDADES` → `INDICADORES.rankingActividades`). Todo lo monta `montarRanking(lista, prefijo)` con ids `<prefijo>-seg` / `<prefijo>-tabla` (`rank`, `rankInv`, `rankAct`). De los 3 botones inferiores, el primero se llama **"Pliego MVCS"**. La fila PLIEGO de la hoja `MVCS` (`INDICADORES.mvcs[0]`) queda solo como resumen del detalle de Pliego MVCS; el desglose Actividades/Proyectos con decimales vive solo en el detalle.
