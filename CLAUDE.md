...
- **Indicador principal de Inicio** (desde 18-ago-2026): título "Sector", subtítulo "Vivienda, Construcción y Saneamiento"; el % grande sale de la nueva hoja `INDICADOR PRINCIPAL` (fila SECTOR → `INDICADORES.principal.pct`, hoy 70,8% → **redondeado a entero** 71%), sin línea secundaria y sin botón de ojo. El botón dice **"Ver ranking"** (sigue abriendo `screen-pliego`). De los 3 botones inferiores, el primero se llama **"Pliego MVCS"**. La fila PLIEGO de la hoja `MVCS` (`INDICADORES.mvcs[0]`) queda solo como resumen del detalle de Pliego MVCS; el desglose Actividades/Proyectos con decimales vive solo en el detalle.
...
- **Detalle de Pliego MVCS** (`screen-pliego`, título "Pliego MVCS"): hoja **`MVCS`** (antes `PLIEGO`), exportada como `INDICADORES.mvcs` (renombrada de `pliego` el 18-ago-2026 para no confundirla con `pliegos`). Se abre desde "Ver ranking" y desde el acceso "Pliego MVCS".
